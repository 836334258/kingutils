var r = {
    795: (r, e, t) => {
      function n(r, e) {
        if (r === e) return !0
        if (r?.length !== e?.length) return !1
        for (let t = 0; t < r.length; t++) if (r[t] !== e[t]) return !1
        return !0
      }
      t.r(e), t.d(e, { default: () => n })
    },
    258: (r, e, t) => {
      t.r(e), t.d(e, { default: () => n })
      const n = (r) => {
        const e = r ? r.length : 0
        return e ? r[e - 1] : void 0
      }
    },
    409: (r, e, t) => {
      t.r(e), t.d(e, { default: () => n })
      const n = (r, e) => {
        const t = []
        if (!Array.isArray(r)) return t
        let n = -1
        const o = r.length
        for (; n++ < o; ) e(r[n], n, r) && t.push(r[n])
        return t
      }
    },
    350: (r, e, t) => {
      t.r(e), t.d(e, { default: () => n })
      const n = (r) => Array.from(new Set(r))
    },
    642: (r, e, t) => {
      var n = {
        './array/arrayEqual.ts': 795,
        './array/arrayLast.ts': 258,
        './array/arrayRemove.ts': 409,
        './array/arrayUnique.ts': 350,
      }
      function o(r) {
        var e = a(r)
        return t(e)
      }
      function a(r) {
        if (!t.o(n, r)) {
          var e = new Error("Cannot find module '" + r + "'")
          throw ((e.code = 'MODULE_NOT_FOUND'), e)
        }
        return n[r]
      }
      ;(o.keys = function () {
        return Object.keys(n)
      }),
        (o.resolve = a),
        (r.exports = o),
        (o.id = 642)
    },
  },
  e = {}
function t(n) {
  var o = e[n]
  if (void 0 !== o) return o.exports
  var a = (e[n] = { exports: {} })
  return r[n](a, a.exports, t), a.exports
}
;(t.d = (r, e) => {
  for (var n in e)
    t.o(e, n) &&
      !t.o(r, n) &&
      Object.defineProperty(r, n, { enumerable: !0, get: e[n] })
}),
  (t.o = (r, e) => Object.prototype.hasOwnProperty.call(r, e)),
  (t.r = (r) => {
    'undefined' != typeof Symbol &&
      Symbol.toStringTag &&
      Object.defineProperty(r, Symbol.toStringTag, { value: 'Module' }),
      Object.defineProperty(r, '__esModule', { value: !0 })
  })
var n = {}
;(() => {
  t.d(n, { Z: () => o })
  const r = t(642),
    e = {}
  r.keys().forEach((t) => {
    const n = t.substring(t.lastIndexOf('/') + 1, t.lastIndexOf('.'))
    e[n] = r(t).default ?? r(t)
  })
  const o = e
})()
var o = n.Z
export { o as default }
//# sourceMappingURL=kingutils.min.js.map
